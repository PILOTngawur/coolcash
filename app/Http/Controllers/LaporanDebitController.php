...
class LaporanDebitController extends Controller
{
    ...
    public function check(Request $request)
    {
        ...
        $debit = Debits::select('debit.id', 'debit.category_id', 'debit.user_id', 'debit.nominal', 'debit.debit_date', 'debit.description', 'categories_debit.id as id_category', 'categories_debit.name')
    ...
    public function export(Request $request)
{
    ...
    $debit = Debits::select('debit.id', 'debit.category_id', 'debit.user_id', 'debit.nominal', 'debit.debit_date', 'debit.description', 'categories_debit.name')
        ->join('categories_debit', 'debit.category_id', '=', 'categories_debit.id', 'LEFT')
        ->whereDate('debit.debit_date', '>=', $tanggal_awal)
        ->whereDate('debit.debit_date', '<=', $tanggal_akhir)
        ->get();

    return view('account.laporan_debit.export', compact('debit', 'tanggal_awal', 'tanggal_akhir'));
}
